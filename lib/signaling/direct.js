...
class DirectSignaling extends SignalingAPI {
  ...
  async sendOfferTo (offer) {
    ...
    // console.log(this.parent.getNeighboursOutview().map(p => p.id), this.parent.getNeighboursInview().map(p => p.id))
    this.parent.send(this.parent.options.n2n.protocol, offer.destination, offer).then(() => {
